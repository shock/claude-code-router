...
3. **Add Session Context**
   ...
   - Handle session boundaries and resets via existing session reset mechanisms
   - **Session Reset Integration**:
     ```typescript
     // Integrate with existing session reset patterns
     // In src/index.ts, add cost reset to session reset logic
...
- Automatic cleanup on session end via LRU eviction
- Support for session reset on new conversations using existing session boundaries
...
- Integrate with session start/end events using existing patterns
- Support session reset functionality via existing session boundaries
- Maintain session cost persistence using separate LRU cache with same capacity and patterns
- **Session Reset Integration**: Hook into existing session reset events to reset cost tracking
